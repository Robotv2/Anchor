...
public abstract class AbstractUserLongTest {
    ...
    @Test
    void testBackendCapabilitiesMatchRepositoryInterfaces() {
        Assertions.assertTrue(database.supports(SupportType.WRAPPED_ASYNC), "Database should expose wrapped async access");
        Assertions.assertFalse(database.supports(SupportType.ASYNC), "No storage backend exposes native async access");
        Assertions.assertEquals(repository instanceof QueryableRepository, database.supports(SupportType.QUERY), "Query capability should match the repository adapter");
        Assertions.assertEquals(repository instanceof MigrationExecutor, database.supports(SupportType.MIGRATION), "Schema Migration capability should match the repository adapter");
    }
